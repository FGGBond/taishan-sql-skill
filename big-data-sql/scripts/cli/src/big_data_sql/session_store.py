...
DEFAULT_SESSION_DIR = Path.home() / ".config" / "big-data-sql"
DEFAULT_SESSION_FILE = DEFAULT_SESSION_DIR / "auth-session.json"
DEFAULT_CACHE_TTL_SECONDS = 4 * 3600


def session_path() -> Path:
    raw = os.getenv("BDP_SQL_SESSION_PATH", "").strip()
    return Path(raw).expanduser() if raw else DEFAULT_SESSION_FILE


def cache_enabled() -> bool:
    raw = os.getenv("BDP_SQL_COOKIE_CACHE", "1").strip().lower()
    return raw not in {"0", "false", "no", "off"}


def cache_ttl_seconds() -> int:
    raw = os.getenv("BDP_SQL_COOKIE_CACHE_TTL", str(DEFAULT_CACHE_TTL_SECONDS)).strip()
    try:
        ttl = int(raw)
    except ValueError:

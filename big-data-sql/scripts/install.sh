...
if [[ -x "${VENV_DIR}/bin/python" ]]; then
  ...
  CLI_BIN="${VENV_DIR}/bin/big-data-sql"
  INSTALL_MODE="venv (${VENV_DIR})"
else
  ensure_pip "${PYTHON}"
  upgrade_build_tools "${PYTHON}"
  install_editable "${PYTHON}"
  if command -v big-data-sql >/dev/null 2>&1; then
    CLI_BIN="$(command -v big-data-sql)"
    INSTALL_MODE="pip (PATH)"
  else
    USER_BASE="$("${PYTHON}" -m site --user-base)"
    if [[ -x "${USER_BASE}/bin/big-data-sql" ]]; then
      CLI_BIN="${USER_BASE}/bin/big-data-sql"
      INSTALL_MODE="pip (--user, ${USER_BASE}/bin)"
    else
      CLI_BIN="${PYTHON} -m big_data_sql"
      INSTALL_MODE="pip (module fallback)"
    fi
  fi
fi

chmod +x "${SCRIPT_DIR}/big-data-sql" 2>/dev/null || true

echo ""
echo "big-data-sql installed via ${INSTALL_MODE}."
echo "Preferred entrypoint: bash scripts/big-data-sql doctor"

    ...
    def call(
        ...
        *,
        ...
        try:
            ...
            return failure(tool_name, "INVALID_ARGUMENT", str(exc), recoverable=False)
        except AuthError as exc:
            return failure(tool_name, "AUTH_UNAVAILABLE", str(exc))
        except HTTPError as exc:
            return failure(tool_name, "HTTP_ERROR", f"HTTP {exc.code}: {exc.reason}", status=exc.code)
        except URLError as exc:
            return failure(tool_name, "NETWORK_ERROR", str(exc.reason))
        except TimeoutError as exc:
            return failure(tool_name, "TIMEOUT", str(exc))
        ...
        try:
            ...
            payload = failure(tool_name, "INVALID_JSON", "接口返回不是有效 JSON", body=body)
            if verbose:
                payload["elapsed_ms"] = elapsed_ms
            return payload

        normalized = normalize_api_response(spec.raw, raw, tool=tool_name, include_raw=include_raw)
        if verbose:
